...
# 计算排序一致性
def plot_simplified_rank_consistency():
    # 确保列名正确
    ...
    # 计算排名
    edit_ranks = np.argsort(np.argsort(-1 * edit_scores))  # 降序排列
    gpt_ranks = np.argsort(np.argsort(-1 * gpt_scores))    # 降序排列
    
    # 计算相关系数
    spearman_corr, _ = spearmanr(edit_scores, gpt_scores)
    kendall_corr, _ = kendalltau(edit_scores, gpt_scores)
    ...
    # 添加相关系数文本（不含p值）
    correlation_text = f'Spearman: {spearman_corr:.2f}\nKendall: {kendall_corr:.2f}'
    ...
    # 保存图片
    plt.savefig('visualizations/simplified_rank_consistency.png', dpi=300, bbox_inches='tight')
    print("Simplified rank consistency visualization saved to 'visualizations/simplified_rank_consistency.png'")
    plt.close()

if __name__ == "__main__":
    plot_simplified_rank_consistency()